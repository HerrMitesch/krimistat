...
# Create DataFrame
city_coords = pd.DataFrame(city_data)


# Load Germany Map Data
germany_map = gpd.read_file("https://raw.githubusercontent.com/johan/world.geo.json/master/countries/DEU.geo.json")

# Merge with City Coordinates (Assumed Available)
#city_coords = pd.DataFrame({
#    'Stadt': df_cases['Stadt'].unique(),
#    'lat': [random.uniform(47, 55) for _ in range(len(df_cases['Stadt'].unique()))],
#    'lon': [random.uniform(6, 15) for _ in range(len(df_cases['Stadt'].unique()))]
#})

df_cases = df_cases.merge(city_coords, on='Stadt', how='left')
...
# Map of Germany with Clickable Cities
st.subheader("Crime Cases by City")
fig_map = px.scatter_mapbox(df_filtered_cases, lat='lat', lon='lon', size='Faelle', hover_name='Stadt',
                            title="Crime Cases Map", mapbox_style="carto-positron", zoom=5)
